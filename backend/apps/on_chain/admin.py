...
class TokenAdmin(admin.ModelAdmin):
    list_display = [
        'name', 'token_id', 'charity_link', 'value_fiat_lkr', 
        'transactions_count', 'total_volume'
    ]
    ...
    fieldsets = (
        ('Token Information', {
            'fields': ('token_id', 'name', 'description')
        }),
        ('Financial Details', {
            'fields': ('value_fiat_lkr', 'charity')
        }),
        ('Statistics', {
    ...
    def transactions_count(self, obj):
        ...
        return format_html('<a href="{}?token__id__exact={}">{} transactions</a>', url, obj.id, count)
...
class OnChainTransactionAdmin(admin.ModelAdmin):
    list_display = [
        'transaction_hash_short', 'token_link', 'from_address_short', 
        'to_address_short', 'amount', 'timestamp'
    ]
    list_filter = ['token', 'timestamp']
    search_fields = [
        'transaction_hash', 'from_address', 'to_address', 
        'token__name', 'token__token_id'
    ]
    ...
    fieldsets = (
        ('Transaction Details', {
            'fields': ('transaction_hash', 'token', 'amount')
        }),
        ('Addresses', {
