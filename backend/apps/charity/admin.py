...
class CharityAdmin(admin.ModelAdmin):
    list_display = [
        'name', 'contact_email', 'campaigns_count', 
        'total_raised', 'contract_address_short', 'created_at'
    ]
    list_filter = ['created_at', 'updated_at']
    search_fields = ['name', 'description', 'contact_email', 'contract_address']
    readonly_fields = ['id', 'created_at', 'updated_at', 'campaigns_count', 'total_raised']
    fieldsets = (
        ('Basic Information', {
        ...
        ('Statistics', {
            'fields': ('campaigns_count', 'total_raised'),
            'classes': ('collapse',)
        }),
        ('Timestamps', {
    ...
    def campaigns_count(self, obj):
        ...
        return format_html('<a href="{}?charity__id__exact={}">{} campaigns</a>', url, obj.id, count)
...
class CampaignAdmin(admin.ModelAdmin):
    list_display = [
        'title', 'charity_link', 'goal_amount', 'raised_amount', 
        'progress_bar', 'status', 'donations_count', 'start_date', 'end_date'
    ]
    list_filter = [
        'charity', 'start_date', 'end_date', 'created_at'
    ]
    search_fields = ['title', 'description', 'charity__name', 'contract_address']
    readonly_fields = [
        'id', 'created_at', 'updated_at', 'progress_percentage', 
        'donations_count', 'status'
    ]
    ...
    def progress_bar(self, obj):
        if obj.goal_amount > 0:
            percentage = min((float(obj.raised_amount) / float(obj.goal_amount)) * 100, 100)
    ...
    def donations_count(self, obj):
        ...
        return format_html('<a href="{}?campaign__id__exact={}">{} donations</a>', url, obj.id, count)
...
class DonationAdmin(admin.ModelAdmin):
    list_display = [
        'user_link', 'campaign_link', 'amount', 'token_link', 
        'token_quantity', 'status_badge', 'donation_timestamp'
    ]
    list_filter = [
        'status', 'campaign__charity', 'token', 'donation_timestamp', 'created_at'
    ]
    search_fields = [
        'user__email', 'user__name', 'campaign__title', 
        'campaign__charity__name', 'token__name'
    ]
    readonly_fields = [
        'id', 'created_at', 'donation_timestamp'
    ]
    ...
    fieldsets = (
        ('Donation Details', {
            'fields': ('user', 'campaign', 'status')
        }),
        ('Amount Information', {
            'fields': ('amount', 'token', 'token_quantity')
        }),
        ('Timestamps', {
            'fields': ('donation_timestamp', 'created_at'),
            'classes': ('collapse',)
        }),
        ('System', {
            'fields': ('id',),
